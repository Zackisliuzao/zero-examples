...
var (
	...
	passCounter         = collection.NewRollingWindow(func() *collection.Bucket[float64] { return new(collection.Bucket[float64]) }, 50, time.Millisecond*100)
	rtCounter           = collection.NewRollingWindow(func() *collection.Bucket[float64] { return new(collection.Bucket[float64]) }, 50, time.Millisecond*100)
	index               int32
)
